...
# input a imagine and make a 5*5*64 to 64 with stride=1*1, and activate with relu
h_conv3 = tf.nn.relu(conv2d(h_pool2, W_conv3) + b_conv3)  # output size 7*7*64
...
for i in range(4000):
    ...
    _, loss_val = sess.run([train_step, cross_entropy], feed_dict={x: batch_x, y: batch_y, keep_prob: 0.5})
    if i % 50 == 0:
...
save_path = saver.save(sess, "./model/MyCNN/MyCNN.ckpt")
accuracy = compute_accuracy(mnist.test.images, mnist.test.labels)
print("result accuracy %g" % compute_accuracy(mnist.test.images, mnist.test.labels))
plt.plot(his_acc)
plt.xlabel('step')
